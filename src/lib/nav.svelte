  ...
  onMount(async () => {
    try {
      ...
      if (user) {
        ...
        const unsubscribe = vinylBalance.subscribe(value => {
            vinyls = value;
          });
  ...
  <nav>
    <div class="left-nav">
      ...
      >
        ...
        <a href="/discuss" class="nav-link" class:active={$page.url.pathname.startsWith("/discuss")}>
          Discuss
        </a>
        <a href="/games" class="nav-link" class:active={$page.url.pathname.startsWith("/games")}>
          {#if user}
          <!-- Vinyls Amount Display -->
          <span class="vinyls-amount">
            Games ({vinyls} Vinyls)
          </span>
          {:else}
          Games
